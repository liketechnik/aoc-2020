...
fn main() -> Result<()> {
    ...
    let inputs = (0..=99).map(|input1| (0..=99).map(move |input2| (input1, input2))).flatten();

    for (input1, input2) in inputs {
        let mut program = program.clone();
        program.input(input1, input2)?;

        let result = program.eval()?;
        if result == 19690720 {
            println!("noun={}, verb={}, answer={}", input1, input2, 100 * input1 + input2);
        }
    }
